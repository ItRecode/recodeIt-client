...
### 1. recordTextArea과 recordInput 두개 모두가 채워져 있어야 아래에 추가버튼이 활성화된다.

부모에서 애내 내용이 먼지 알필요있나?

### 2. formData를 전송할 때 양식이 두개 있는데, 일반적인 객체값과 파일을 담는값

### 3. 수정모드 일때와 일반 추가모드일때 UI가 다르다.

### 4. 레코드타입이 달라지면 UI가 바뀌어야한다.(일반모드일떄만)

### 5. 레코드카테고리를 백에서 데이터받고 앞선 레코드타입에 따라서 값을만들어서 UI를 보여준다.
